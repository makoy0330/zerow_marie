...
except IndexError:  # In case getUnconnectedOutLayers() returns a scalar
...
while True:
    ...
    for out in outs:
        for detection in out:
            ...
            if confidence > 0.5:  # You can adjust this threshold to filter detections
    ...
    if len(indexes) > 0:
        for i in indexes.flatten():
            x, y, w, h = boxes[i]
            label = str(classes[class_ids[i]])  # Get the label from coco.names
